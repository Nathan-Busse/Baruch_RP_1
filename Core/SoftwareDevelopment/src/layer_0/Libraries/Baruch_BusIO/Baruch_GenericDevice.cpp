...
Baruch_GenericDevice::Baruch_GenericDevice(
...
/*! @brief Simple begin function (doesn't do much at this time)
    @return true always
*/
bool Baruch_GenericDevice::begin(void) {
  _begun = true;
  return true;
}
...
*/
void Baruch_GenericDevice::end(void) { _begun = false; }

/*! @brief Write a buffer of data
   @param buffer Pointer to buffer of data to write
   @param len Number of bytes to write
   @return true if write was successful, otherwise false */
bool Baruch_GenericDevice::write(const uint8_t *buffer, size_t len) {
  if (!_begun)
    return false;
...
bool Baruch_GenericDevice::read(uint8_t *buffer, size_t len) {
  if (!_begun)
    return false;
...
/*! @brief Read from a register location
   ...
   @return true if read was successful, otherwise false */
bool Baruch_GenericDevice::readRegister(uint8_t *addr_buf, uint8_t addrsiz,
...
/*! @brief Write to a register location
   ...
   @return true if write was successful, otherwise false */
bool Baruch_GenericDevice::writeRegister(uint8_t *addr_buf, uint8_t addrsiz,
                                           const uint8_t *buf,
                                           uint16_t bufsiz) {
