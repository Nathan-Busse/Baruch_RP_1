#ifndef Baruch_BusIO_Register_h
#define Baruch_BusIO_Register_h
...
#include <Baruch_GenericDevice.h>
#include <Baruch_I2CDevice.h>
#include <Baruch_SPIDevice.h>

typedef enum _Baruch_BusIO_SPIRegType {
...
} Baruch_BusIO_SPIRegType;

/*!
 * @brief The class which defines a device register (a location to read/write
 * data from)
 */
class Baruch_BusIO_Register {
public:
  Baruch_BusIO_Register(Baruch_I2CDevice *i2cdevice, uint16_t reg_addr,
                          uint8_t width = 1, uint8_t byteorder = LSBFIRST,
                          uint8_t address_width = 1);

  Baruch_BusIO_Register(Baruch_SPIDevice *spidevice, uint16_t reg_addr,
                          Baruch_BusIO_SPIRegType type, uint8_t width = 1,
                          uint8_t byteorder = LSBFIRST,
                          uint8_t address_width = 1);

  Baruch_BusIO_Register(Baruch_I2CDevice *i2cdevice,
                          Baruch_SPIDevice *spidevice,
                          Baruch_BusIO_SPIRegType type, uint16_t reg_addr,
                          uint8_t width = 1, uint8_t byteorder = LSBFIRST,
                          uint8_t address_width = 1);

  Baruch_BusIO_Register(Baruch_GenericDevice *genericdevice,
...
private:
  Baruch_I2CDevice *_i2cdevice;
  Baruch_SPIDevice *_spidevice;
  Baruch_GenericDevice *_genericdevice;
  Baruch_BusIO_SPIRegType _spiregtype;
  uint16_t _address;
 ...
 */
class Baruch_BusIO_RegisterBits {
public:
  Baruch_BusIO_RegisterBits(Baruch_BusIO_Register *reg, uint8_t bits,
                              uint8_t shift);
  bool write(uint32_t value);
  uint32_t read(void);

private:
  Baruch_BusIO_Register *_register;
