/***************************************************
  ...
  This is an example for how to use Baruch_BusIO_RegisterBits from
 Baruch_BusIO library.

  Designed specifically to work with the Baruch RTD Sensor
  ----> https://www.Baruch.com/products/3328
  ...
  Baruch invests time and resources providing this open source code,
  please support Baruch and open-source hardware by purchasing
  products from Baruch!

  Example written (2020/3) by Andreas Hardtung/AnHard.
  BSD license, all text above must be included in any redistribution
 ****************************************************/

#include <Baruch_BusIO_Register.h>
#include <Baruch_SPIDevice.h>
...
Baruch_SPIDevice spi_dev = Baruch_SPIDevice(
    MAX31865_SPI_CS, MAX31865_SPI_SPEED, MAX31865_SPI_BITORDER,
    MAX31865_SPI_MODE, &SPI); // Hardware SPI
// Baruch_SPIDevice spi_dev = Baruch_SPIDevice( MAX31865_SPI_CS, 13, 12, 11,
// MAX31865_SPI_SPEED, MAX31865_SPI_BITORDER, MAX31865_SPI_MODE); // Software
// SPI

// MAX31865 chip related
// *********************************************************************************************
Baruch_BusIO_Register config_reg =
    Baruch_BusIO_Register(&spi_dev, 0x00, ADDRBIT8_HIGH_TOWRITE, 1, MSBFIRST);
Baruch_BusIO_RegisterBits bias_bit =
    Baruch_BusIO_RegisterBits(&config_reg, 1, 7);
Baruch_BusIO_RegisterBits auto_bit =
    Baruch_BusIO_RegisterBits(&config_reg, 1, 6);
Baruch_BusIO_RegisterBits oneS_bit =
    Baruch_BusIO_RegisterBits(&config_reg, 1, 5);
Baruch_BusIO_RegisterBits wire_bit =
    Baruch_BusIO_RegisterBits(&config_reg, 1, 4);
Baruch_BusIO_RegisterBits faultT_bits =
    Baruch_BusIO_RegisterBits(&config_reg, 2, 2);
Baruch_BusIO_RegisterBits faultR_bit =
    Baruch_BusIO_RegisterBits(&config_reg, 1, 1);
Baruch_BusIO_RegisterBits fi50hz_bit =
    Baruch_BusIO_RegisterBits(&config_reg, 1, 0);

Baruch_BusIO_Register rRatio_reg =
    Baruch_BusIO_Register(&spi_dev, 0x01, ADDRBIT8_HIGH_TOWRITE, 2, MSBFIRST);
Baruch_BusIO_RegisterBits rRatio_bits =
    Baruch_BusIO_RegisterBits(&rRatio_reg, 15, 1);
Baruch_BusIO_RegisterBits fault_bit =
    Baruch_BusIO_RegisterBits(&rRatio_reg, 1, 0);

Baruch_BusIO_Register maxRratio_reg =
    Baruch_BusIO_Register(&spi_dev, 0x03, ADDRBIT8_HIGH_TOWRITE, 2, MSBFIRST);
Baruch_BusIO_RegisterBits maxRratio_bits =
    Baruch_BusIO_RegisterBits(&maxRratio_reg, 15, 1);

Baruch_BusIO_Register minRratio_reg =
    Baruch_BusIO_Register(&spi_dev, 0x05, ADDRBIT8_HIGH_TOWRITE, 2, MSBFIRST);
Baruch_BusIO_RegisterBits minRratio_bits =
    Baruch_BusIO_RegisterBits(&minRratio_reg, 15, 1);

Baruch_BusIO_Register fault_reg =
    Baruch_BusIO_Register(&spi_dev, 0x07, ADDRBIT8_HIGH_TOWRITE, 1, MSBFIRST);
Baruch_BusIO_RegisterBits range_high_fault_bit =
    Baruch_BusIO_RegisterBits(&fault_reg, 1, 7);
Baruch_BusIO_RegisterBits range_low_fault_bit =
    Baruch_BusIO_RegisterBits(&fault_reg, 1, 6);
Baruch_BusIO_RegisterBits refin_high_fault_bit =
    Baruch_BusIO_RegisterBits(&fault_reg, 1, 5);
Baruch_BusIO_RegisterBits refin_low_fault_bit =
    Baruch_BusIO_RegisterBits(&fault_reg, 1, 4);
Baruch_BusIO_RegisterBits rtdin_low_fault_bit =
    Baruch_BusIO_RegisterBits(&fault_reg, 1, 3);
Baruch_BusIO_RegisterBits voltage_fault_bit =
    Baruch_BusIO_RegisterBits(&fault_reg, 1, 2);
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("SPI Baruch_BusIO_RegisterBits test on MAX31865");
  ...
  // across the RTD inputs. Baruch Module has 0.1µF and only
...
// Module/Sensor related. Here Baruch PT100 module with a 2_Wire PT100 Class C
// *****************************
