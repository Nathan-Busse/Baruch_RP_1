...
Baruch_SPIDevice spi_dev = Baruch_SPIDevice(SPIDEVICE_CS);
...
void setup() {
  ...
  Baruch_BusIO_Register id_reg =
      Baruch_BusIO_Register(&spi_dev, 0x0F, ADDRBIT8_HIGH_TOREAD);
  uint8_t id = 0;
  id_reg.read(&id);
  Serial.print("ID register = 0x");
  Serial.println(id, HEX);

  Baruch_BusIO_Register thresh_reg = Baruch_BusIO_Register(
