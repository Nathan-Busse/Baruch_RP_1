 ...
 */
Baruch_SPIDevice::Baruch_SPIDevice(int8_t cspin, uint32_t freq,
 ...
 *    @param  dataMode The SPI mode to use, defaults to SPI_MODE0
 */
Baruch_SPIDevice::Baruch_SPIDevice(int8_t cspin, int8_t sckpin,
 ...
 */
Baruch_SPIDevice::~Baruch_SPIDevice() {
  if (_spiSetting)
    delete _spiSetting;
}
 ...
 * init
 */
bool Baruch_SPIDevice::begin(void) {
  if (_cs != -1) {
    pinMode(_cs, OUTPUT);
 ...
 */
void Baruch_SPIDevice::transfer(uint8_t *buffer, size_t len) {
  //
  // HARDWARE SPI
  //
  if (_spi) {
 ...
 */
uint8_t Baruch_SPIDevice::transfer(uint8_t send) {
  uint8_t data = send;
  transfer(&data, 1);
  return data;
}
 ...
 * SPI)
 */
void Baruch_SPIDevice::beginTransaction(void) {
  if (_spi) {
#ifdef BUSIO_HAS_HW_SPI
 ...
 */
void Baruch_SPIDevice::endTransaction(void) {
  if (_spi) {
#ifdef BUSIO_HAS_HW_SPI
    _spi->endTransaction();
#endif
 ...
 */
void Baruch_SPIDevice::setChipSelect(int value) {
  if (_cs != -1) {
 ...
 */
void Baruch_SPIDevice::beginTransactionWithAssertingCS() {
  beginTransaction();
  setChipSelect(LOW);
}
 ...
 */
void Baruch_SPIDevice::endTransactionWithDeassertingCS() {
  setChipSelect(HIGH);
  endTransaction();
}
 ...
 * writes
 */
bool Baruch_SPIDevice::write(const uint8_t *buffer, size_t len,
 ...
 * writes
 */
bool Baruch_SPIDevice::read(uint8_t *buffer, size_t len, uint8_t sendvalue) {
 ...
 * writes
 */
bool Baruch_SPIDevice::write_then_read(const uint8_t *write_buffer,
 ...
 * writes
 */
bool Baruch_SPIDevice::write_and_read(uint8_t *buffer, size_t len) {
  beginTransactionWithAssertingCS();
  transfer(buffer, len);
  endTransactionWithDeassertingCS();
