 ...
 * to 1 byte
 */
Baruch_BusIO_Register::Baruch_BusIO_Register(Baruch_I2CDevice *i2cdevice,
                                                 uint16_t reg_addr,
                                                 uint8_t width,
                                                 uint8_t byteorder,
 ...
 * to 1 byte
 */
Baruch_BusIO_Register::Baruch_BusIO_Register(Baruch_SPIDevice *spidevice,
                                                 uint16_t reg_addr,
                                                 Baruch_BusIO_SPIRegType type,
                                                 uint8_t width,
                                                 uint8_t byteorder,
                                                 uint8_t address_width) {
 ...
 * to 1 byte
 */
Baruch_BusIO_Register::Baruch_BusIO_Register(
    Baruch_I2CDevice *i2cdevice, Baruch_SPIDevice *spidevice,
    Baruch_BusIO_SPIRegType type, uint16_t reg_addr, uint8_t width,
 ...
 */
Baruch_BusIO_Register::Baruch_BusIO_Register(
    Baruch_GenericDevice *genericdevice, uint16_t reg_addr, uint8_t width,
    uint8_t byteorder, uint8_t address_width) {
  _i2cdevice = nullptr;
  _spidevice = nullptr;
 ...
 * uncheckable)
 */
bool Baruch_BusIO_Register::write(uint8_t *buffer, uint8_t len) {
 ...
 * uncheckable)
 */
bool Baruch_BusIO_Register::write(uint32_t value, uint8_t numbytes) {
  if (numbytes == 0) {
    numbytes = _width;
  }
  if (numbytes > 4) {
 ...
 */
uint32_t Baruch_BusIO_Register::read(void) {
 ...
 *    @return Returns 0xFFFFFFFF on failure, value otherwise
 */
uint32_t Baruch_BusIO_Register::readCached(void) { return _cached; }
...
*/
bool Baruch_BusIO_Register::read(uint8_t *buffer, uint8_t len) {
  uint8_t addrbuffer[2] = {(uint8_t)(_address & 0xFF),
                           (uint8_t)(_address >> 8)};
  if (_i2cdevice) {
 ...
 * uncheckable)
 */
bool Baruch_BusIO_Register::read(uint16_t *value) {
  if (!read(_buffer, 2)) {
    return false;
  }
 ...
 * uncheckable)
 */
bool Baruch_BusIO_Register::read(uint8_t *value) {
  if (!read(_buffer, 1)) {
    return false;
  }
 ...
 */
void Baruch_BusIO_Register::print(Stream *s) {
  uint32_t val = read();
  s->print("0x");
  s->print(val, HEX);
}
 ...
 *    @param  s The Stream to print to, defaults to &Serial
 */
void Baruch_BusIO_Register::println(Stream *s) {
  print(s);
  s->println();
}
 ...
 * touching other bits
 *    @param  reg The Baruch_BusIO_Register which defines the bus/register
 *    @param  bits The number of bits wide we are slicing
 *    @param  shift The number of bits that our bit-slice is shifted from LSB
 */
Baruch_BusIO_RegisterBits::Baruch_BusIO_RegisterBits(
    Baruch_BusIO_Register *reg, uint8_t bits, uint8_t shift) {
  _register = reg;
  _bits = bits;
  _shift = shift;
}
 ...
 */
uint32_t Baruch_BusIO_RegisterBits::read(void) {
 ...
 * uncheckable)
 */
bool Baruch_BusIO_RegisterBits::write(uint32_t data) {
  uint32_t val = _register->read();
 ...
 */
uint8_t Baruch_BusIO_Register::width(void) { return _width; }

/*!
 *    @brief  Set the default width of data
 *    @param width the default width of data read from register
 */
void Baruch_BusIO_Register::setWidth(uint8_t width) { _width = width; }

/*!
 *    @brief  Set register address
 *    @param address the address from register
 */
void Baruch_BusIO_Register::setAddress(uint16_t address) {
  _address = address;
}
 ...
 */
void Baruch_BusIO_Register::setAddressWidth(uint16_t address_width) {
  _addrwidth = address_width;
}
